...
def create_storage_backend(data_dir: Path) -> StorageBackend:
    """
    Creates storage backend based on environment variables.
    
    Environment Variables:
    - STORAGE_BACKEND: json|sqlite|postgres|git (default: json)
    - DATABASE_URL: Database connection string (used for sqlite/postgres)
    - GIT_REPO_URL: Git repository URL (used for git)
    - GIT_TOKEN: Git access token (used for git)
    - GIT_BRANCH: Git branch (default: main)
    - GIT_FILE_PATH: File path in Git repository (default: accounts.json)
    """
...
def _mask_password(url: str) -> str:
    """Hides password in database connection string"""
    if "://" not in url:
        return url
    try:
