...
export type RegisterConfig = {
  ...
  proxy: string;
  proxy_rotating_enabled: boolean;
  proxy_rotating_keys: string[];
  total: number;
  threads: number;
...
export function mapAccountToFE(account: any): Account {
  if (!account) return account;
  return {
    ...account,
    status: STATUS_MAP_TO_FE[account.status] || account.status,
  };
}

export function mapAccountMutationResponseToFE(res: any): AccountMutationResponse {
  if (!res) return res;
  return {
    ...res,
...
export function mapRefreshProgressResponseToFE(res: any): RefreshProgressResponse {
  if (!res) return res;
  const mapped = { ...res };
  ...
  if (mapped.result) {
    mapped.result = {
      ...mapped.result,
      items: Array.isArray(mapped.result.items) ? mapped.result.items.map(mapAccountToFE) : [],
    };
  }
  if (Array.isArray(mapped.results)) {
    mapped.results = mapped.results.map((r: any) => ({
      ...r,
      status: STATUS_MAP_TO_FE[r.status] || r.status,
    }));
  }
  return mapped;
}
...
export async function fetchAccounts() {
  const res = await httpRequest<AccountListResponse>("/api/accounts");
  return {
    ...res,
    items: Array.isArray(res.items) ? res.items.map(mapAccountToFE) : [],
  };
...
export async function createAccounts(tokens: string[], accounts: AccountImportPayload[] = []) {
  const mappedAccounts = accounts.map((acc) => {
    const mapped = { ...acc };
    if (mapped.status) {
      mapped.status = STATUS_MAP_TO_BE[mapped.status as string] || mapped.status;
    }
    return mapped;
  });
  const res = await httpRequest<AccountMutationResponse>("/api/accounts", {
    method: "POST",
    body: { tokens, accounts: mappedAccounts },
  });
...
export async function finishOAuthLogin(sessionId: string, callback: string) {
  const res = await httpRequest<AccountMutationResponse>("/api/accounts/oauth/finish", {
    method: "POST",
...
export async function fetchRefreshProgress(progressId: string) {
  const res = await httpRequest<RefreshProgressResponse>(`/api/accounts/refresh/progress/${progressId}`);
...
export async function fetchReLoginProgress(progressId: string) {
  const res = await httpRequest<RefreshProgressResponse>(`/api/accounts/re-login/progress/${progressId}`);
  return mapRefreshProgressResponseToFE(res);
}
...
export async function updateAccount(
  ...
  if (mappedUpdates.status) {
    mappedUpdates.status = STATUS_MAP_TO_BE[mappedUpdates.status] as any;
  }
...
export async function createImageEditTask(clientTaskId: string,
  files: File | File[],
  prompt: string,
  model?: ImageModel,
  size?: string,
...
export async function fetchManagedImages(filters: { start_date?: string; end_date?: string }) {
  ...
  return httpRequest<{ items: ManagedImage[]; groups: Array<{ date: string; items: ManagedImage[] }> }>(`/api/images${params.toString()? `?${params.toString()}`: ""}`,);
}
...
export async function deleteToTarget(targetFreeMb: number) {
  return httpRequest<{ removed: number; freed_mb: number; done: boolean }>(`/api/images/storage/cleanup-to-target?target_free_mb=${targetFreeMb}&dry_run=false`,
    { method: "POST" },);
}
...
export async function updateCPAPool(poolId: string,
...
export async function updateSub2APIServer(serverId: string,
  updates: {
    name?: string;
    base_url?: string;
    email?: string;
