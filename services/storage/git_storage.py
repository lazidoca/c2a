...
class GitStorageBackend(StorageBackend):
    """Git private repository storage backend"""
    ...
    def _build_auth_url(repo_url: str, token: str) -> str:
        """Constructs authenticated Git URL"""
        if not token:
            return repo_url
        
        # Support HTTPS format: https://github.com/user/repo.git
        if repo_url.startswith("https://"):
            # Insert token
            return repo_url.replace("https://", f"https://{token}@")
        
        # Support git@ format: git@github.com:user/repo.git
    ...
    def _clone_or_pull(self) -> Repo:
        ...
        if repo_path.exists() and (repo_path / ".git").exists():
            # Repository exists, pull latest code
            try:
                repo = Repo(repo_path)
    ...
    def load_accounts(self) -> list[dict[str, Any]]:
        """Load account data from Git repository"""
        try:
    ...
    def save_accounts(self, accounts: list[dict[str, Any]]) -> None:
        """Save account data to Git repository"""
        try:
    ...
    def load_auth_keys(self) -> list[dict[str, Any]]:
        """Load auth key data from Git repository"""
        try:
    ...
    def save_auth_keys(self, auth_keys: list[dict[str, Any]]) -> None:
        """Save auth key data to Git repository"""
        try:
