...
def compress_images(quality: int = 60) -> dict:
    """Re-compresses all images, returning the saved space."""
    saved = 0
    count = 0
...
def delete_to_target(target_free_mb: int, dry_run: bool = False) -> dict:
    """Deletes the oldest images until the remaining free space reaches target_free_mb."""
    import shutil
    usage = shutil.disk_usage(config.images_dir)
...
def _auto_cleanup_worker(stop_event: threading.Event) -> None:
    """Background thread: checks storage every 30 minutes, automatically cleaning oldest images when space falls below threshold."""
    import shutil
    min_free_mb = getattr(config, "image_min_free_mb", None)
    if min_free_mb is None:
        min_free_mb = 500

    while not stop_event.wait(1800):  # Every 30 minutes
        try:
            config.cleanup_old_images()
            cleanup_image_thumbnails()
            usage = shutil.disk_usage(config.images_dir)
