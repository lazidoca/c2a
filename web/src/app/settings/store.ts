  ...
  testImageStorage: async () => {
    ...
    try {
      ...
      if (data.result.ok) {
        ...
        toast.error(`Kết nối WebDAV không thành công: ${data.result.error?? `HTTP ${data.result.status}`}`);
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Kiểm tra WebDAV không thành công");
    } finally {
