...
def is_tls_connection_error(message: str) -> bool:
    """Detects TLS/SSL connection errors, which can typically be resolved by retrying."""
    text = str(message or "").lower()
    return (
        "curl: (35)" in text
...
def is_connection_timeout_error(message: str) -> bool:
    """Detects connection timeout errors (e.g., curl 28), which can be resolved by a short wait and retry on the same account."""
    text = str(message or "").lower()
    return (
        "curl: (28)" in text
...
# Detects partial tool calls returned by the model (e.g., {"size":"1920x1088","n":1})
# These JSONs contain parameters for the image generation tool but did not actually generate images
...
def is_model_text_reply_instead_of_image(message: str) -> bool:
    """Detects if the model returned a text reply (containing tool call JSON) instead of actually generating an image.

    When upstream ChatGPT fails to trigger the image generation tool, it returns a descriptive text,
    which might contain JSON parameters (such as prompt, referenced_image_ids, size/n, etc.).
    This scenario should be treated as "upstream did not generate images" rather than "content policy violation".

    Detects two modes:
    1. Complete tool call JSON (including referenced_image_ids)
    2. Partial tool parameter JSON (e.g., {"size":"1920x1088","n":1})
    """
    if not message:
        return False
    if REFERENCED_IMAGE_IDS_RE.search(message):
        return True
    # Detects partial tool parameter JSON (model returned parameters but did not trigger tool)
...
def build_image_prompt(prompt: str, size: str | None, quality: str = "auto") -> str:
    hints = []
    if size:
        hints.append(f"Output image size is {size}.")
    if quality:
        hints.append(f"Output image quality is {quality}.")
...
# Used to filter non-image file IDs (like file_upload_business_upsell)
...
def extract_conversation_ids(payload: str) -> tuple[str, list[str], list[str]]:
    ...
    # Extract only genuine image file IDs (file_00000000... format), filtering non-image file IDs (like file_upload_business_upsell)
...
def _is_user_message_event(event: dict[str, Any]) -> bool:
    """Checks if the event is from a user role message."""
    value = event.get("v")
    message = event.get("message") or (value.get("message") if isinstance(value, dict) else None)
...
def _get_detailed_error_from_tasks(
    ...
    """Fetches structured error information from /backend-api/tasks/.

    When the SSE stream detects moderation interception, it polls the tasks endpoint to get detailed error text.
    Uses structured fields (metadata.is_error, author.role, content.content_type) to judge,
    rather than relying on volatile text matching.

    Args:
    - `backend`: OpenAIBackendAPI instance.
    - `conversation_id`: Conversation ID.
    - `timeout_secs`: Request timeout in seconds.
    - `wait_secs`: Seconds to wait for task creation. Set to 0 to skip wait.

    Returns:
    - Detailed error message text, empty string if not found.
    """
...
def stream_image_outputs(
    ...
    if message and not file_ids and not sediment_ids and last.get("blocked"):
        # Attempt to get detailed error information from /backend-api/tasks/
    ...
    # Detect if the model returned a text description (including referenced_image_ids) instead of actually generating an image
    # This indicates the model initiated the image generation tool call, but the SSE disconnected before the tool finished,
    # meaning the image might be generating asynchronously. A more aggressive polling strategy is needed.
    ...
    # When a text reply is detected but conversation_id is missing, try to recover it from the recent conversation list
    # When the SSE stream is too short (model returns text instead of triggering image tool), conversation_id might not be captured,
    # but the image has been asynchronously generated upstream. Recover conversation_id by listing recent conversations.
    ...
    # Before polling for the image, check if there is a moderation block via /backend-api/tasks/
    # This avoids unnecessary long polling timeouts
    # Note: when should_poll_for_image is True or a text reply is detected,
    # even if tasks reports an "error", we cannot return immediately—because upstream might mark tool call JSON parameters
    # (e.g., {"size":"1792x1024","n":1}) as is_error, while the image is actually being generated asynchronously.
    # In this case, we should continue polling for the image.
    detailed_error = ""
    ...
    # When a text reply is detected (including referenced_image_ids), use a longer timeout to poll for the image result.
    # Because upstream might execute image generation as an async task, and the SSE stream disconnected before the tool finished,
    # leading to the image tool response record not yet being written to the conversation document.
    poll_timeout = config.image_poll_timeout_secs
    if is_text_reply and conversation_id:
        # In text reply scenarios, the image might still be generating asynchronously; use a longer timeout (default 120s -> extra 180s = 300s)
    ...
    try:
        ...
        # In text reply scenarios, task error should not be immediately judged as a content policy violation,
        # as the image might still be generating asynchronously in the background
        ...
        # When text reply is detected, transient network errors during the first poll should not abort immediately,
        # as the image might still be generating asynchronously in the background, subsequent retry polls will continue.
        if is_text_reply and conversation_id:
            logger.warning({
    ...
    if message:
        # Detect if the model returned a text description (including referenced_image_ids) instead of actually generating an image
        # This indicates the model initiated the image generation tool call, but the SSE disconnected before the tool finished.
        # In this case, we should attempt to poll for the image result again, rather than treating the text as the final output.
        # When is_text_reply is True but conversation_id is missing, try to recover from the recent conversation list
        if is_text_reply and not conversation_id:
            try:
                import time as _time
                recovered_id = backend.find_conversation_by_prompt(
        ...
        if is_text_reply and conversation_id:
            ...
            # In text reply scenarios, the image may take 4-5 minutes to finish generating asynchronously.
            # Use a 300s timeout and allow multiple retries to avoid early exits due to transient network issues.
            ...
            for poll_attempt in range(1, MAX_POLL_RETRIES + 1):
                try:
                    ...
                    # If there are remaining retries and it is not a timeout/content policy error, continue retrying
                    if poll_attempt < MAX_POLL_RETRIES and not isinstance(exc, (ImagePollTimeoutError, ImageContentPolicyError)):
                        ...
                        time.sleep(backoff)
                        continue
                    # Timeout error or retries exhausted, stop retrying
                    break
    ...
    # Fallback: when message is empty and image URL parsing fails, first try a short-delay retry poll
    # and then throw a clear error instead of letting the caller receive a vague "upstream completed without generating images"
    logger.warning({
    ...
    # When should_poll_for_image is True but conversation_id is missing, try to recover
    ...
    if should_poll_for_image and conversation_id:
        # The image might still be processing asynchronously (upstream SSE stream finished before image generation completed).
        # Use a 300s timeout and allow multiple retries to avoid early exit due to transient network issues or image not yet submitted.
        retry_poll_timeout = max(config.image_poll_timeout_secs, 300)
        ...
        # Still failed after retries, yield error message
        ...
        # If conversation_id is also empty (SSE stream extremely short, session ID not captured),
        # still yield a message to avoid stream_image_outputs_with_pool producing
        # the vague "upstream completed without generating images" error
        yield ImageOutput(kind="message", model=request.model, index=index, total=total,
...
def _generate_single_image(
    ...
    """Performs generation logic for a single image (including retries), returning a list of results.

    This function runs in a separate thread, and each thread uses a different account,
    enabling parallel image generation to prevent serial timeout blocking.
    """
    # Maximum retries when model returns text instead of image
    MAX_TEXT_REPLY_RETRIES = 3
    # Maximum retries for TLS connection errors
    MAX_TLS_RETRIES = 3
    # Maximum retries for connection timeouts (same account short wait retry)
    MAX_CONN_TIMEOUT_RETRIES = 3
    # Maximum retries for polling timeouts (switch account and retry)
    ...
    while True:
        ...
        try:
            ...
            # Polling timeout: switch account and retry
            ...
            error_text = str(exc)
            # If model returns text instead of image, try switching account and retry
            ...
            if not emitted_for_token and is_token_invalid_error(last_error):
                ...
                continue
            # TLS/SSL connection error: auto retry
            if not emitted_for_token and is_tls_connection_error(last_error):
                ...
                if tls_retry_count <= MAX_TLS_RETRIES:
                    ...
                    continue
            # Connection timeout error (curl 28): same account short wait retry, do not switch account
...
def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    """Generates multiple images in parallel, each using a separate thread and account, non-blocking."""
    ...
    if request.n <= 1:
        # Single image, execute directly (no thread pool overhead required)
        outputs = _generate_single_image(request, 1, 1)
        for output in outputs:
            yield output
        return

    # Multiple images: choose parallel or serial execution based on configuration
    ...
    # One thread per image, started simultaneously
    futures = {}
    ...
    # yield results: skip index order limit, preventing low-index failures from blocking high-index successes
    emitted = False
    last_error = ""
    # First yield all successful results
    for index in range(1, request.n + 1):
        if index in results:
    ...
    # If there are failures but also successes, log a warning
    if emitted:
        for index in range(1, request.n + 1):
            if index in errors:
                logger.warning({
