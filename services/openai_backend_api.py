...
class ChatRequirements:
    """Saves the sentinel token required for a single conversation request."""
    token: str
    proof_token: str = ""
    turnstile_token: str = ""
    so_token: str = ""
...
DEFAULT_CLIENT_VERSION = "prod-497f333866796e100096ad083b51ca949d22e751"
...
EDITABLE_FILE_PPT_PROMPT = """I need you to create an editable PPT based on the user's requirements. You can use an Agent to do this. Do not ask the user any more questions. You can supplement the content style, layout, color scheme, content structure, and page information on your own and execute directly. The overall process is as follows:
1. Generate an exquisite product introduction PPT of 5-6 pages using image generation.
2. Separate all involved image and shape assets into individual PNG files, one image per asset without omission, so that I can directly splice the assets in the PPT to restore it, without text.
3. Using all the above images and shape assets, help me restore the presentation PPT you generated the first time. I need it in an editable PPT format, with the main parts restored and inserted separately, and the text must be editable.
Finally, only generate a PPT file and a ZIP archive of all the assets encountered during generation."""
EDITABLE_FILE_PSD_PROMPT = "Generate this image for me. Separate this poster into multiple images including the background image. Do not change the position of each element so that I can directly use it without having to drag them. The background color should be white, do not use a fake transparent background. Then merge the above split images into a PSD file, remove the white background, do not change the corresponding position of each layer, preserve the corresponding position of the layer where each element resides, and preserve the layer of each element. Finally, only output the PSD file and a ZIP file of each layer."
...
# Genuine image file ID format: file_00000000 + 24-character hex (32 characters total)
...
# Content policy violation keywords (various refusal expressions returned by upstream when blocking image generation)
_CONTENT_POLICY_KEYWORDS = (
    # Explicit content policy violation
    "content policy", "safety restrictions", "safety system", "violated", "violation", "moderation", "policy", "blocked",
    # Refusal to generate
    "cannot generate", "can't generate", "unable to generate", "cannot help", "can't help", "unable to help",
    # Sensitive content categories
    "nudity", "naked", "exposed", "pornography", "porn", "nsfw", "explicit", "sexual content", "sexual", "minor", "minors", "underage",
    # General refusal
...
def _is_content_policy_error(error_msg: str) -> bool:
    """Check if the error message indicates a content policy violation."""
    if not error_msg:
        return False
...
class OpenAIBackendAPI:
    ...
    Note:
    - When `access_token` is passed, chat and model list endpoints use the logged-in path.
      E.g., `/backend-api/sentinel/chat-requirements`, `/backend-api/conversation`
    - When `access_token` is not passed, they use the anonymous path.
    ...
    - Protocol compatibility conversions are located in `services.protocol`
    """
    ...
    def __init__(self, access_token: str = "") -> None:
        ...
        Args:
        - `access_token`: Optional. If passed, uses the logged-in path; otherwise, uses the anonymous path.
        """
        ...
        self.session.headers.update({
            ...
            "Sec-Ch-Ua-Full-Version-List": '"Chromium";v="148.0.0.0", "Microsoft Edge";v="148.0.0.0", "Not/A)Brand";v="99.0.0.0"',
    ...
    def _get_offset_for_timezone(self, tz_name: str) -> int:
        """Get the offset in minutes (following JavaScript format) for a specified IANA timezone name at the current time."""
        try:
            tz = zoneinfo.ZoneInfo(tz_name)
            dt = datetime.datetime.now(tz)
            offset_seconds = dt.utcoffset().total_seconds()
    ...
    def _build_fp(self) -> Dict[str, str]:
        ...
        fp.setdefault("impersonate", "chrome146")
        fp.setdefault("oai-device-id", new_uuid())
        fp.setdefault("oai-session-id", new_uuid())
        fp.setdefault("sec-ch-ua", '"Chromium";v="148", "Microsoft Edge";v="148", "Not/A)Brand";v="99"')
    ...
    def _headers(self, path: str, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        """Constructs request headers, appending target path/route required by web client."""
    ...
    def _get_conversation_init(self) -> Dict[str, Any]:
        ...
        _, tz_offset = self._get_timezone_and_offset()
    ...
    def _get_default_account(self) -> Dict[str, Any]:
        ...
        _, tz_offset = self._get_timezone_and_offset()
        response = self.session.get(self.base_url + path + f"?timezone_offset_min={tz_offset}", headers=self._headers(path),
                                    timeout=20)
        if response.status_code != 200:
            self._raise_on_error(response, path)
    ...
    def get_user_info(self) -> Dict[str, Any]:
        """Retrieves account information of the current token."""
    ...
    def _bootstrap_headers(self) -> Dict[str, str]:
        """Constructs request headers for home page pre-heating."""
        return {
            "User-Agent": self.user_agent,
    ...
    def _build_requirements(self, data: Dict[str, Any], source_p: str = "") -> ChatRequirements:
        """Organizes sentinel response into token sets required for subsequent conversations."""
    ...
    def _conversation_headers(self, path: str, requirements: ChatRequirements) -> Dict[str, str]:
        """Constructs conversation SSE request headers based on current requirements."""
        headers = {
    ...
    def _api_messages_to_conversation_messages(self, messages: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        """Converts standard chat messages to messages required by web conversation."""
    ...
    def _image_headers(self, path: str, requirements: ChatRequirements, conduit_token: str = "", accept: str = "*/*") -> \
            Dict[str, str]:
        """Constructs image path request headers."""
        headers = {
            "Content-Type": "application/json",
            "Accept": accept,
            "OpenAI-Sentinel-Chat-Requirements-Token": requirements.token,
    ...
    def _prepare_image_conversation(self, prompt: str, requirements: ChatRequirements, model: str) -> str:
        """Prepares conduit token for image generation."""
        path = "/backend-api/f/conversation/prepare"
        tz_name, tz_offset = self._get_timezone_and_offset()
        payload = {
            "action": "next",
    ...
    def _decode_image_base64(self, image: str) -> bytes:
        """Decodes base64 image string or local path into binary."""
        if (
                image
                and len(image) < 512
    ...
    def _upload_image(self, image: str, file_name: str = "image.png") -> Dict[str, Any]:
        """Uploads a base64 image, returning the underlying file metadata."""
    ...
    def _start_image_generation(self, prompt: str, requirements: ChatRequirements, conduit_token: str, model: str,
        ...
        """Initiates SSE request for image generation or editing."""
    ...
    def delete_conversation(self, conversation_id: str) -> Dict[str, Any]:
        """Deletes local conversation history."""
        path = f"/backend-api/conversation/{conversation_id}"
    ...
    def _list_recent_conversations(self, limit: int = 5, timeout_secs: float = 10.0) -> list[Dict[str, Any]]:
        """Lists recent conversations, sorted by update time desc.

        When the SSE stream is too short and conversation_id is lost, this method
        can be used to find the recently created conversation and recover it.
        """
    ...
    def find_conversation_by_prompt(self, prompt: str, started_at: float, timeout_secs: float = 10.0) -> str:
        """Finds matching conversation_id from recent conversation list based on prompt and start time.

        Used to recover conversation_id when SSE stream is too short.
        Matches by comparing prompt keywords and timestamps.

        Args:
            prompt: User input prompt text
            started_at: Request start timestamp (epoch seconds)
            timeout_secs: Request timeout in seconds

        Returns:
            Matched conversation_id, empty string if not found.
        """
        items = self._list_recent_conversations(limit=10, timeout_secs=timeout_secs)
        if not items:
            return ""
        # Filter conversations created before or around started_at (up to 5 minutes ago)
        # ChatGPT's updated_at is usually later than actual request time
        prompt_lower = str(prompt or "").lower().strip()
        best_match = ""
        best_score = 0.0
        for item in items:
            # item might be a full conversation object or a summary
            conv_id = str(item.get("id") or item.get("conversation_id") or "")
            if not conv_id:
                continue
            # Check time range: conversation's updated_at should be after (or around) request start time
            ...
            # Image-to-Image titles usually start with "Image"
        ...
        if best_match and best_score > 0.1:
            ...
            return best_match
        # If no title matches, return the latest conversation (most recent time)
        for item in items:
            conv_id = str(item.get("id") or item.get("conversation_id") or "")
            updated_at = float(item.get("update_time") or item.get("updated_at") or 0)
    ...
    def _editable_prompt(fixed_prompt: str, user_prompt_text: str) -> str:
        ...
        return fixed_prompt if not extra else fixed_prompt + "\n\nFollowing are supplementary requirements, please integrate and execute directly:\n" + extra
    ...
    def _extract_image_reference_ids(cls, payload: Any) -> tuple[list[str], list[str]]:
        ...
        def walk(value: Any) -> None:
            if isinstance(value, str):
                # Extract only genuine image file IDs (file_00000000... format) and file-service:// URIs
    ...
    def _extract_image_tool_records(self, data: Dict[str, Any]) -> list[Dict[str, Any]]:
        """Extracts image tool output records from conversation details."""
    ...
    def _find_content_policy_error_in_conversation(data: Dict[str, Any]) -> str:
        """Finds content policy violation error messages in conversation document.

        When upstream rejects image generation, error message appears in assistant message text.
        Iterates through assistant/tool messages, checking for content policy violation keywords,
        returning matched message text (truncated to 500 chars) or empty string.
        """
        mapping = data.get("mapping") or {}
    ...
    def _poll_image_results(
        ...
        while _remaining() > 0:
            attempt += 1
            # On each poll, check if /backend-api/tasks/ has errors (log only, do not interrupt)
            # Content policy violation detection is done via conversation text (in _find_content_policy_error_in_conversation)
            last_task_error = ""
            try:
                ...
                # tasks query failure does not affect normal polling flow
                logger.debug({
            ...
            # Check if conversation text contains content policy violation errors
            # When upstream rejects image generation, error message appears in assistant message,
            # not in /backend-api/tasks/ task error structure.
            # If content policy violation is detected without image file ID, interrupt polling immediately.
            ...
            if file_ids or sediment_ids:
                if not config.image_check_before_hit_enabled:
                    # Check-before-hit mechanism disabled: return first discovered file_ids directly
                ...
                if not config.image_settle_enabled:
                    # Double confirmation mechanism disabled: return first discovered file_ids directly
        ...
        exc = ImagePollTimeoutError(
            f"ChatGPT image generation timed out (waited {timeout_secs} seconds)."
            f"You can increase image_poll_timeout_secs in config.json,"
            f"or it might be due to account rate limits or image queue congestion."
        )
        if last_task_error:
    ...
    def _get_attachment_download_url(self, conversation_id: str, attachment_id: str) -> str:
        """Gets download URL via conversation attachment endpoint."""
        path = f"/backend-api/conversation/{conversation_id}/attachment/{attachment_id}/download"
        response = self.session.get(self.base_url + path, headers=self._headers(path, {"Accept": "application/json"}),
                                    timeout=60)
        ensure_ok(response, path)
    ...
    def _query_backend_tasks(
        ...
        """Queries /backend-api/tasks/ to get async task status and error info.

        Args:
        - `conversation_id`: Optional. Filter tasks by conversation_id.
        - `task_id`: Optional. Filter tasks by task_id.
        - `timeout_secs`: Request timeout in seconds.

        Returns:
        - List of tasks, each containing image_gen_message etc.
        """
        ...
        # Filter by conversation_id or task_id
        if conversation_id:
            tasks = [
                t for t in tasks
    ...
    def check_task_error(self, task: Dict[str, Any]) -> tuple[bool, str, Dict[str, Any]]:
        """Checks if a single task contains structured errors.

        Determined by fields (does not rely on text matching):
        - image_gen_message.metadata.is_error == True
        - image_gen_message.author.role == "assistant" (not "tool")
        - image_gen_message.content.content_type == "text" (not "multimodal_text")
    ...
    def _resolve_image_urls(self, conversation_id: str, file_ids: list[str], sediment_ids: list[str]) -> list[str]:
        """Parses image result id into downloadable URL."""
        urls = []
    ...
    def resolve_conversation_image_urls(
        ...
        # When check-before-hit and settle are both disabled and SSE has provided file_ids,
        # skip polling and resolve URL directly to save initial_wait + polling time.
        ...
        if poll and conversation_id:
            ...
            try:
                ...
                # If polling times out and there is a task error (like moderation block), raise ImageContentPolicyError
                # instead of ImagePollTimeoutError, letting caller distinguish genuine timeout from upstream rejection
    ...
    def stream_conversation(
        ...
        logger.info(f"[Timing] bootstrap step in stream_conversation took: {t_boot_end - t_boot_start:.4f}s")

        t_req_start = time.perf_counter()
        requirements = self._get_chat_requirements()
        t_req_end = time.perf_counter()
        logger.info(f"[Timing] chat requirements step in stream_conversation took: {t_req_end - t_req_start:.4f}s")

        path, timezone = self._chat_target()
        payload = self._conversation_payload(normalized, model, timezone, thinking_effort=thinking_effort)
        
        t_post_start = time.perf_counter()
        _headers = self._conversation_headers(path, requirements)
        response = self.session.post(
            self.base_url + path,
            headers=_headers,
            json=payload,
        ...
        logger.info(f"[DEBUG] chat completion POST response: {response}")
        ensure_ok(response, path)
        t_post_end = time.perf_counter()
        logger.info(f"[Timing] chat completion POST response header received in: {t_post_end - t_post_start:.4f}s")

        try:
            t_stream_start = time.perf_counter()
            yield from iter_sse_payloads(response)
            t_stream_end = time.perf_counter()
            logger.info(f"[Timing] chat completion stream finished. Stream duration: {t_stream_end - t_stream_start:.4f}s")
        finally:
            response.close()
    ...
    def _bootstrap(self) -> None:
        """Pre-heats home page and extracts PoW script references."""
        response = self.session.get(
            self.base_url + "/",
    ...
    def _get_chat_requirements(self) -> ChatRequirements:
        """Gets sentinel token required for current conversation mode (prepare + finalize two-step flow)."""
        t_total_start = time.perf_counter()
        base = "/backend-api/sentinel/chat-requirements" if self.access_token else "/backend-anon/sentinel/chat-requirements"
        
        t_p_token_start = time.perf_counter()
        p_token = build_legacy_requirements_token(self.user_agent, self.pow_script_sources, self.pow_data_build)
        t_p_token_end = time.perf_counter()
        logger.info(f"[Timing] build_legacy_requirements_token took: {t_p_token_end - t_p_token_start:.4f}s")
        ...
        logger.info(f"[Timing] chat_requirements_prepare POST took: {t_prep_end - t_prep_start:.4f}s")
        ...
        if proof_info.get("required"):
            ...
            t_pow_end = time.perf_counter()
            logger.info(f"[Timing] build_proof_token (seed={proof_info.get('seed')}, diff={proof_info.get('difficulty')}) took: {t_pow_end - t_pow_start:.4f}s")
        ...
        if turnstile_info.get("required") and turnstile_info.get("dx"):
            ...
            t_ts_end = time.perf_counter()
            logger.info(f"[Timing] solve_turnstile_token took: {t_ts_end - t_ts_start:.4f}s")
        ...
        logger.info(f"[Timing] chat_requirements_finalize POST took: {t_final_end - t_final_start:.4f}s")
        ...
        logger.info(f"[Timing] _get_chat_requirements total: {t_total_end - t_total_start:.4f}s")
        return ChatRequirements(
            token=token,
            proof_token=proof_token,
            turnstile_token=turnstile_token,
    ...
    def _chat_target(self) -> tuple[str, str]:
        tz_name, _ = self._get_timezone_and_offset()
        if self.access_token:
            return "/backend-api/conversation", tz_name
        return "/backend-anon/conversation", tz_name

    def list_models(self) -> Dict[str, Any]:
        """Returns available models for current mode, format aligned with OpenAI `/v1/models`."""
        self._bootstrap()
