...
DEFAULT_REVIEW_PROMPT = "Determine if the user request is allowed. Reply only with ALLOW or REJECT."
...
def check_request(text: str) -> None:
    ...
    for word in config.sensitive_words:
        if word in text:
            raise HTTPException(status_code=400, detail={"error": "Phát hiện từ nhạy cảm, từ chối thực hiện tác vụ này."})
    ...
    def _on_failure(event_payload: dict) -> None:
        ...
        if not fail_open:
            raise HTTPException(
                status_code=503,
                detail={"error": "Dịch vụ kiểm duyệt AI tạm thời không khả dụng, vui lòng thử lại sau."},
            )
    ...
    if _is_reject_decision(decision):
        raise HTTPException(status_code=400, detail={"error": "Kiểm duyệt AI không thông qua, từ chối thực hiện tác vụ này."})
