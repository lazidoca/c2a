...
def _clean(value: object, default: str = "") -> str:
    """Cleans a string: converts to string and strips leading/trailing whitespace."""
    text = str(value if value is not None else default).strip()
    return text or default


def _is_upload(value: object) -> TypeGuard[UploadFile]:
    """Identifies uploaded file: compatible with UploadFile returned by Starlette forms."""
    return isinstance(value, UploadFile)


def _parse_bool(value: object) -> bool | None:
    """Parses boolean field: compatible with JSON boolean and form string values."""
...
def _parse_count(value: object) -> int:
    """Parses generation count: enforces the 1 to 4 limit on the image interface."""
    try:
...
def _payload_from_fields(fields: dict[str, Any]) -> dict[str, Any]:
    """Constructs image editing payload: extracts common parameters from form or JSON fields."""
...
def _json_reference_value(value: object) -> object:
    """Parses form image reference: supports writing the images field as a JSON string."""
...
def _source_from_object(value: dict[str, Any]) -> list[ImageSource]:
    """Extracts image reference object: supports image_url or url, explicitly rejects file_id."""
...
def _sources_from_value(value: object) -> list[ImageSource]:
    """Expands image reference: normalizes strings, arrays, and objects into a list of image sources."""
...
def _json_image_sources(body: dict[str, Any]) -> list[ImageSource]:
    """Reads JSON image reference: prioritizes official images array field."""
    sources: list[ImageSource] = []
...
async def parse_image_edit_request(request: Request) -> tuple[dict[str, Any], list[ImageSource], list[ImageSource]]:
    """Parses image edit request: supports both multipart uploads and official JSON image URLs.
    
    Returns (payload, image_sources, mask_sources)
    """
...
def _extension_from_mime(mime_type: str) -> str:
    """Infers image extension: converts MIME type to a common file suffix."""
...
def _safe_filename(name: str, mime_type: str, fallback: str) -> str:
    """Generates safe filename: cleans URL filename and appends extension."""
...
def _decode_data_url(url: str) -> ImageInput:
    """Decodes data URL: converts inline image to standard image input tuple."""
...
def _response_mime_type(response: requests.Response, parsed_path: str) -> str:
    """Identifies downloaded image type: prioritizes response header, falls back to URL suffix inference."""
...
def _filename_from_url(parsed_path: str, mime_type: str) -> str:
    """Generates URL image filename: extracts name from link path and sanitizes it."""
...
def _download_image_url(url: str) -> ImageInput:
    """Downloads remote image: converts http/https image link to standard image input tuple."""
    source = _clean(url)
...
async def read_image_sources(sources: list[ImageSource]) -> list[ImageInput]:
    """Reads image sources: uploads are read directly, URLs are downloaded first, returning a list of image tuples."""
