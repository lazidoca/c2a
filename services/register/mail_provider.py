...
# in_use exceeding this seconds is considered stale (leftover from crash), can be re-claimed
...
def _load_outlook_token_state() -> dict[str, dict[str, Any]]:
    """Reads mailbox pool status file, returns {email_lower: {state, reason, updated_at}}.

    Backward-compatible: plain string lists (historical "used emails") are interpreted as used.
    """
    try:
...
def _outlook_entry_available(entry: dict[str, Any] | None) -> bool:
    """Whether the email is currently available for use: unregistered, stale in_use, or non-terminal."""
...
def _release_outlook_token_state(address: str) -> None:
    """Releases in_use back to unused (only if currently indeed in_use)."""
    target = str(address or "").strip().lower()
    if not target:
        return
    with _outlook_token_state_lock:
...
def reset_outlook_token_pool_state(scope: str = "all") -> int:
    """Resets mailbox pool status file.

    scope=all clears all records; scope=failed only clears failed/token_invalid/in_use (retains used).
...
def outlook_token_pool_stats(pool: list[dict[str, str]] | None = None) -> dict[str, int]:
    """Counts mailbox pool states. pool is the currently imported list (used to calculate unused)."""
    store = _load_outlook_token_state()
...
def _next_domain(domains: list[str]) -> str:
    ...
    if not domains:
        raise RuntimeError("mail.domain cannot be empty")
    if len(domains) == 1:
        return domains[0]
    with domain_lock:
...
class CloudflareTempMailProvider(BaseMailProvider):
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        if not address or not token:
            raise RuntimeError("CloudflareTempMail is missing address or jwt")
    ...
    def get_existing_mailbox(self, email: str) -> dict[str, Any]:
        """Retrieves the JWT of an existing mailbox using the admin password, used to query mail."""
        data = self._request("POST", "/admin/get_address", headers={"x-admin-auth": self.admin_password}, payload={"address": email})
        address = str(data.get("address") or "").strip()
        token = str(data.get("jwt") or "").strip()
        if not address or not token:
            raise RuntimeError(f"CloudflareTempMail could not retrieve JWT for existing mailbox {email}")
        return {"provider": self.name, "provider_ref": self.provider_ref, "address": address, "token": token}
...
class DDGMailProvider(BaseMailProvider):
    ...
    def _cf_request(self, method: str, path: str, headers: dict | None = None, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)) -> dict:
        ...
        if resp.status_code not in expected:
            raise RuntimeError(f"DDGMail CF request failed: {method} {path}, HTTP {resp.status_code}, body={resp.text[:300]}")
        return {} if resp.status_code == 204 else resp.json()
    ...
    def _ddg_request(self, method: str, path: str, payload: dict | None = None) -> dict:
        ...
        if resp.status_code not in (200, 201):
            raise RuntimeError(f"DDG API request failed: {method} {path}, HTTP {resp.status_code}, body={resp.text[:300]}")
        return resp.json()
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        if not ddg_address_part:
            raise RuntimeError("DDG API response is missing address field")
        ddg_address = f"{ddg_address_part}@duck.com"

        if _is_ddg_alias_duplicate(ddg_address):
            raise RuntimeError(f"[{self.label}] DDG daily limit reached, alias {ddg_address} already exists, automatically switching mail providers")

        _record_ddg_alias(ddg_address)

        if not self.cf_inbox_jwt:
            raise RuntimeError("DDGMail requires cf_inbox_jwt (fixed inbox JWT for DDG forwarding target), please specify CF Inbox JWT in mail configuration")
...
class CloudMailGenProvider(BaseMailProvider):
        ...
        for attempt in range(attempts):
            try:
                ...
                if resp.status_code in expected:
                    return {} if resp.status_code == 204 else resp.json()
                message = f"CloudMailGen request failed: {method} {path}, HTTP {resp.status_code}, body={resp.text[:300]}"
                ...
                last_error = f"CloudMailGen request exception: {method} {path}, error={error}"
            if attempt < attempts - 1:
                time.sleep(0.5 * (attempt + 1))
        raise RuntimeError(last_error or f"CloudMailGen request failed: {method} {path}")
    ...
    def _fetch_email_list(self, token: str, address: str) -> dict:
        ...
        if not isinstance(data, dict):
            raise RuntimeError(f"CloudMailGen emailList returned exception: {data}")
        return data

    def _get_token(self) -> str:
        if not self.admin_email or not self.admin_password:
            raise RuntimeError("CloudMailGen is missing admin_email or admin_password")
        ...
        if not token:
            raise RuntimeError(f"CloudMailGen genToken returned exception: {data}")
        with cloudmail_token_lock:
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        if not self.domain:
            raise RuntimeError("CloudMailGen requires at least one domain to be configured")
    ...
    def fetch_latest_message(self, mailbox: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not address:
            raise RuntimeError("CloudMailGen is missing address")
        token = self._get_token()
...
class TempMailLolProvider(BaseMailProvider):
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)):
        ...
        data = resp.json()
        if not isinstance(data, dict):
            raise RuntimeError(f"TempMail.lol {method} {path} response structure is not an object")
        return data
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        if not address or not token:
            raise RuntimeError("TempMail.lol is missing address or token")
        return {"provider": self.name, "provider_ref": self.provider_ref, "address": address, "token": token}
...
class MoEmailProvider(BaseMailProvider):
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)):
        ...
        data = resp.json()
        if not isinstance(data, dict):
            raise RuntimeError(f"MoEmail {method} {path} response structure is not an object")
        return data
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        if not address or not email_id:
            raise RuntimeError("MoEmail is missing email or id")
    ...
    def fetch_latest_message(self, mailbox: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not email_id:
            raise RuntimeError("MoEmail is missing email_id")
...
class InbucketMailProvider(BaseMailProvider):
    ...
    def _resolve_domain(self) -> str:
        ...
        raise RuntimeError("Inbucket requires at least one domain to be configured")
    ...
    def fetch_latest_message(self, mailbox: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not mailbox_name:
            raise RuntimeError("Inbucket is missing mailbox_name")
...
class YydsMailProvider(BaseMailProvider):
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        if not address or not token:
            raise RuntimeError("YYDSMail is missing address or token")
...
class OutlookTokenError(RuntimeError):
    """Failed to exchange refresh_token for access_token (invalid credentials/permissions), distinguished from "failed to read mail"."""
...
def _normalize_outlook_pool(value: Any) -> list[dict[str, str]]:
    """Mailbox pool supports both plain text (one entry per line) and parsed object lists."""
    if isinstance(value, str):
...
class OutlookTokenProvider(BaseMailProvider):
    ...
    def _exchange_refresh_token(self, client_id: str, refresh_token: str, scope: str) -> str:
        ...
        if not access_token:
            raise OutlookTokenError("OutlookToken refresh response is missing access_token")
        return access_token

    def _access_token(self, mailbox: dict[str, Any], client_id: str, refresh_token: str, scope: str) -> str:
        """Caches access_token for reuse: avoids exchanging token on every wait_for_code poll to prevent rate limiting."""
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        with _outlook_token_state_lock:
            ...
            if credential is None:
                raise RuntimeError(f"[{self.label}] OutlookToken mailbox pool has no available emails (total {len(self.pool)} emails, all used, occupied, or invalid), please import new emails or reset pool status")
    ...
    def _graph_messages(self, mailbox: dict[str, Any], access_token: str) -> list[dict[str, Any]]:
        """Returns the most recent N emails (Graph is sorted by receivedDateTime desc, newest first)."""
    ...
    def _imap_messages(self, mailbox: dict[str, Any], access_token: str) -> list[dict[str, Any]]:
        """Returns the most recent N emails, newest first."""
    ...
    def fetch_recent_messages(self, mailbox: dict[str, Any]) -> list[dict[str, Any]]:
        """Fetches the most recent N emails (newest first) for wait_for_code to scan for verification codes."""
        client_id = str(mailbox.get("client_id") or "").strip()
        refresh_token = str(mailbox.get("refresh_token") or "").strip()
        if not client_id or not refresh_token:
            raise RuntimeError("OutlookToken mailbox is missing client_id or refresh_token")
        errors: list[str] = []
    ...
    def wait_for_code(self, mailbox: dict[str, Any]) -> str | None:
        """Iterates through the most recent N emails during polling, extracting verification codes from each to avoid missing the code if the newest email is an ad or security alert."""
        seen_value = mailbox.setdefault("_seen_code_message_refs", [])
        if not isinstance(seen_value, list):
            seen_value = []
            mailbox["_seen_code_message_refs"] = seen_value
...
def _enabled_entries(mail_config: dict) -> list[dict]:
    ...
    if not items:
        raise RuntimeError("No enabled providers in mail.providers")
    return items
...
def create_mailbox(mail_config: dict, username: str | None = None) -> dict:
    ...
    for _ in range(len(enabled)):
        ...
        try:
            ...
            if "DDG daily limit reached" not in last_error and "DDG日上限已达" not in last_error:
                raise
        finally:
            provider.close()
...
def mark_mailbox_result(mailbox: dict, *, success: bool, error: Exception | str | None = None) -> None:
    """Updates mailbox pool status after the registration flow finishes.

    Only applies to outlook_token mailboxes: marks success as used; if failure is due to token invalidation, marks token_invalid;
    other failures are marked failed (keeping the mailbox occupied for troubleshooting, can be released via reset).
    """
    ...
    if isinstance(error, OutlookTokenError) or "OutlookToken refresh failed" in reason or "OutlookToken 刷新失败" in reason or "access_token" in reason:
...
def release_mailbox(mailbox: dict) -> None:
    """Releases outlook_token mailbox from in_use back to unused (when flow is aborted and verification code is not consumed)."""
    if str(mailbox.get("provider") or "") != OutlookTokenProvider.name:
        return
    ...
    for _ in range(len(enabled)):
        ...
        try:
            ...
            if hasattr(provider, "get_existing_mailbox"):
                ...
                raise RuntimeError(f"Mail provider {provider.name} does not support querying existing mailboxes")
        except RuntimeError as error:
            last_error = str(error)
            if "DDG daily limit reached" not in last_error and "DDG日上限已达" not in last_error:
                raise
            ...
            provider.close()
    raise RuntimeError(last_error or "All enabled mail providers failed to query existing mailbox")
