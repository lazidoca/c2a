...
class OAuthLoginStartRequest(BaseModel):
    """Starts the OAuth bridge. email_hint is optional, used only to pre-fill the email on the OpenAI login page."""
    email_hint: str = ""


class OAuthLoginFinishRequest(BaseModel):
    """Submits the callback. callback can be either a full URL or just the code."""
    session_id: str = ""
    callback: str = ""
...
def create_router() -> APIRouter:
    ...
    async def update_user_key(
        ...
        if not updates:
            raise HTTPException(status_code=400, detail={"error": "Chưa phát hiện thay đổi nào, vui lòng sửa đổi trước khi lưu."})
        try:
            item = auth_service.update_key(key_id, updates, role="user")
        except ValueError as exc:
            raise HTTPException(status_code=400, detail={"error": str(exc)}) from exc
        if item is None:
            raise HTTPException(status_code=404, detail={"error": "Khóa người dùng này không tồn tại, có thể đã bị xóa."})
    ...
    async def re_login_accounts(body: AccountRefreshRequest, authorization: str | None = Header(default=None)):
        """Executes the password re-login process for the selected accounts (password login -> verification code login -> token refresh)."""
        require_admin(authorization)
        access_tokens = [str(token or "").strip() for token in body.access_tokens if str(token or "").strip()]
        if not access_tokens:
            raise HTTPException(status_code=400, detail={"error": "access_tokens is required"})
    ...
    async def export_accounts(body: AccountExportRequest, authorization: str | None = Header(default=None)):
        ...
        if not items:
            raise HTTPException(
                status_code=400,
                detail={"error": "Không có tài khoản đầy đủ để xuất, cần có đồng thời access_token, refresh_token và id_token"},
            )
    ...
    async def start_oauth_login(
        ...
        """Registers a PKCE session and returns the authorize URL for the user's browser."""
        require_admin(authorization)
        try:
    ...
    async def finish_oauth_login(
        ...
        """Receives the callback URL / code captured from the user's browser, exchanges it for the token triad, and saves it to disk."""
        require_admin(authorization)
        # Input log: truncate sensitive fields, keeping only the first few characters to facilitate troubleshooting without leaking secrets
