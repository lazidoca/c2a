...
def _serialize_outlook_pool(credentials: list[dict]) -> str:
    return "\n".join(
        f'{c["email"]}|{c.get("password", "")}|{c["refresh_token"]}|{c["client_id"]}' for c in credentials
    )


def _merge_outlook_pool(old_text: str, new_text: str) -> str:
    """Merges stored mailbox pool with newly imported text, deduplicates by email, and overwrites old credentials with new imports of the same email."""
...
def _normalize(raw: dict) -> dict:
    ...
    cfg["proxy_rotating_enabled"] = bool(cfg.get("proxy_rotating_enabled"))
    cfg["proxy_rotating_keys"] = [str(k).strip() for k in cfg.get("proxy_rotating_keys", []) if str(k).strip()]
...
class RegisterService:
    ...
    def _redact_outlook_pools(self, snapshot: dict) -> None:
        """Redacts password/refresh_token from the outlook_token mailbox pool in public output, keeping only masked previews and stats.

        Changes mailboxes to write-only input field (output empty) to prevent broadcasting passwords and refresh_tokens via GET/SSE.
        """
    ...
    def _merge_outlook_pools(self, updates: dict) -> None:
        """For outlook_token provider: merges newly imported mailboxes from frontend with existing pool, deduplicating by email.

        Frontend mailboxes field is a write-only import area; leaving it blank makes no changes. New lines append or overwrite existing credentials.
        Aligned with existing providers of the same type by array index.
        """
    ...
    def update(self, updates: dict) -> dict:
        with self._lock:
            ...
            openai_register.config.update({k: self._config[k] for k in ("mail", "proxy", "proxy_rotating_enabled", "proxy_rotating_keys", "total", "threads")})
            self._save()
            return self.get()
    ...
    def stop(self) -> dict:
        with self._lock:
            ...
            self._save()
            self._append_log("Stop registration task requested, waiting for active tasks to complete", "yellow")
            return self.get()
    ...
    def reset_outlook_pool(self, scope: str = "all") -> dict:
        ...
        if scope == "unused":
            with self._lock:
                ...
                self._save()
                self._append_log(f"Cleared unused emails from Outlook pool, removed {removed}", "yellow")
            return self.get()
        scope = "failed" if str(scope) == "failed" else "all"
        cleared = mail_provider.reset_outlook_token_pool_state(scope)
        with self._lock:
            self._append_log(
                f"Outlook mailbox pool status reset (range={'Failed/Occupied only' if scope == 'failed' else 'All'}), cleared {cleared} records",
                "yellow",
            )
        return self.get()
    ...
    def _target_reached(self, cfg: dict, submitted: int) -> bool:
        ...
        if mode == "quota":
            ...
            self._append_log(f"Checking pool: current normal accounts={metrics['current_available']}, current remaining quota={metrics['current_quota']}, target quota={cfg.get('target_quota')}, {'skipping registration' if reached else 'continuing registration'}", "yellow")
            return reached
        if mode == "available":
            reached = metrics["current_available"] >= int(cfg.get("target_available") or 1)
            self._append_log(f"Checking pool: current normal accounts={metrics['current_available']}, target accounts={cfg.get('target_available')}, current remaining quota={metrics['current_quota']}, {'skipping registration' if reached else 'continuing registration'}", "yellow")
            return reached
    ...
    def _run(self) -> None:
        ...
        with self._lock:
            self._config["enabled"] = False
            self._save()
        self._append_log(f"Registration task finished, success={success}, fail={fail}", "yellow")
