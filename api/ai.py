...
async def filter_or_log(call: LoggedCall, text: str) -> None:
    try:
        ...
        call.log("Yêu cầu thất bại", status="failed", error=str(exc.detail))
        raise
...
def create_router() -> APIRouter:
    ...
    async def generate_images(
        ...
        call = LoggedCall(identity, "/v1/images/generations", body.model, "Văn bản sang hình ảnh", request_text=body.prompt)
    ...
    async def edit_images(
        ...
        call = LoggedCall(identity, "/v1/images/edits", model, "Hình ảnh sang hình ảnh", request_text=prompt)
    ...
    async def create_ppt_task(body: EditableFileTaskRequest, request: Request, authorization: str | None = Header(default=None)):
        identity = require_identity(authorization)
        await filter_or_log(LoggedCall(identity, "/v1/ppt/generations", "gpt-5-5-thinking", "Nhiệm vụ tạo PPT", request_text=body.prompt), body.prompt)
    ...
    async def create_psd_task(body: EditableFileTaskRequest, request: Request, authorization: str | None = Header(default=None)):
        identity = require_identity(authorization)
        await filter_or_log(LoggedCall(identity, "/v1/psd/generations", "gpt-5-5-thinking", "Nhiệm vụ tạo PSD", request_text=body.prompt), body.prompt)
        return await run_in_threadpool(
