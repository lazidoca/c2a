...
class AccountService:
    """Account pool service, saves accounts using token -> account dict."""
    ...
    def _record_token_refresh_error(self, access_token: str, event: str, error: str) -> None:
        ...
        log_service.add(
            LOG_TYPE_ACCOUNT,
            "Failed to refresh access_token using refresh_token",
    ...
    def _apply_refreshed_tokens(self, old_access_token: str, token_data: dict, event: str) -> str:
        ...
        log_service.add(
            LOG_TYPE_ACCOUNT,
            "refresh_token has refreshed access_token",
    ...
    def refresh_access_token(self, access_token: str, *, force: bool = False, event: str = "refresh_access_token") -> str:
        ...
        with self._token_refresh_lock:
            ...
            try:
                ...
                # If app_session_terminated error, try password re-login
                if "app_session_terminated" in error_str.lower():
                    # Get account info (email, password)
                    email = str(account.get("email") or "").strip()
                    password = str(account.get("password") or "").strip()
                    if email and password:
                        # Create new thread to execute password re-login
                        t = Thread(
    ...
    def _password_re_login_thread(self, access_token: str, email: str, password: str, event: str, progress_id: str | None = None) -> None:
        """Password re-login thread entry point"""
        try:
            ...
            if result.get("ok"):
                ...
                # Build token_data for _apply_refreshed_tokens use
                token_data = {
                ...
                # Update account using _apply_refreshed_tokens (handles token aliases)
                new_token = self._apply_refreshed_tokens(access_token, token_data, f"{event}:password_relogin")

                # Additionally update source_type and status (silent, to avoid duplicate logs)
                ...
                if error_type == "password_verify_failed_403" and isinstance(result.get("detail"), dict):
                    ...
                    if isinstance(detail_error, dict) and detail_error.get("code") == "account_deactivated":
                        # Account deleted/deactivated -> mark as disabled
                        self.update_account(access_token, {"status": "禁用", "quota": 0}, quiet=True)
                        account = self.get_account(access_token) or {}
                        log_service.add(
                            LOG_TYPE_ACCOUNT,
                            "Account deactivated - marked as disabled",
                            {
                                "source": event,
                                "token": anonymize_token(access_token),
                                "email": email,
                        ...
                        # Permanent failure: mark account as abnormal (or auto remove)
            ...
            # Mark account as abnormal (or auto remove)
    ...
    def _login_with_password(self, email: str, password: str) -> dict:
        """Login via email + password, returns {access_token, refresh_token, id_token, ...}"""
        from curl_cffi import requests
        ...
        try:
            ...
            # ─── Method 2: OAuth authorize flow ──────────────────────────
            # Using Platform Client + PKCE (same as registration flow)
            
            from utils.pkce import generate_pkce
            code_verifier, code_challenge = generate_pkce()
            
            # ② Initiate OAuth authorize request (using Platform Client + PKCE)
            ...
            # Check if final URL points to error page
            ...
            if not auth_code:
                ...
                if page_type == "email_otp_verification":
                    # Verification code required to log in, directly mark account as abnormal
                    return {"ok": False, "error": "need_verification_code", "detail": login_data}
                else:
                    return {"ok": False, "error": "no_auth_code", "detail": login_data}
            
            # ④ Exchange code for token (using Platform Client + code_verifier, same as registration flow)
    ...
    def get_available_access_token(
        ...
        """Gets an available image generation token from candidate pool.

        Performs preliminary filtering based on local cache, then remote verification via fetch_remote_info (token validity, quota, etc.).
        Limits maximum attempts to prevent infinite loop caused by token rotation.
        """
        ...
        for _attempt in range(max_attempts):
            ...
            try:
                ...
                continue
            # fetch_remote_info internally may cause access_token to change due to token rotation.
            # Add the new token to the exclusion list to prevent duplicate attempts.
    ...
    def list_accounts(self) -> list[dict]:
        """Returns copies of all accounts and appends the current in-flight count image_inflight for each.

        image_inflight is the memory concurrent count (number of images being generated by the account). If account pool is idle
        and this value persists > 0 for an account, it suggests a concurrent slot leak, and the account is silently excluded from dispatch (useful for UI diagnostics).
        """
        with self._lock:
            result = []
            for item in self._accounts.values():
    ...
    def _prepare_account_payload(item: dict) -> dict | None:
        ...
        # type=codex in CPA/Codex exported files is the export format, not the pool package type.
    ...
    def _record_invalid_token_seen(
        ...
        with self._lock:
            ...
            if should_defer:
                log_service.add(
                    LOG_TYPE_ACCOUNT,
                    "Postpone marking abnormal account",
                    {"source": event, "token": anonymize_token(access_token), "error": str(error or "")},
                )
                return False
        return True
    ...
    def init_refresh_progress(self, progress_id: str, total: int) -> None:
        """Initializes refresh progress record."""
    ...
    def update_refresh_progress(self, progress_id: str, token: str) -> None:
        """Updates progress count after refreshing a single account."""
    ...
    def init_relogin_progress(self, progress_id: str, total: int) -> None:
        """Initializes re-login progress record."""
    ...
    def update_relogin_progress(self, progress_id: str, token: str, status: str, error: str | None = None) -> None:
        """Updates a single re-login progress. Automatically marks completed when all accounts are processed."""
        with self._relogin_progress_lock:
    ...
    def refresh_accounts(
        ...
        try:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    error_str = str(exc)
                    # TLS/proxy connection error is network issue, not counted as account failure
        ...
        # Automatically re-login abnormal accounts (only when configuration enabled)
        relogined = 0
    ...
    def re_login_accounts(self, access_tokens: list[str], progress_id: str | None = None) -> dict[str, Any]:
        """Performs password re-login flow on selected accounts.

        Only valid for accounts containing email + password.
        Automatically sets status to "正常" after successful login.
        """
        access_tokens = list(dict.fromkeys(token for token in access_tokens if token))
        if not access_tokens:
        ...
        for token in access_tokens:
            ...
            if not account:
                ...
                if progress_id:
                    self.update_relogin_progress(progress_id, token, "跳过", "Account does not exist")
                continue
            ...
            if not email or not password:
                skipped += 1
                if progress_id:
                    self.update_relogin_progress(progress_id, token, "跳过", "No email password")
                continue

            # Execute password re-login in new thread
            t = Thread(
                target=self._password_re_login_thread,
        ...
        if progress_id:
            # If all accounts are processed synchronously (no threads started), mark completed directly
            if relogined == 0:
                self.finish_relogin_progress(progress_id, result)
            else:
                # Thread running, complete after thread finished
                pass
        return result
