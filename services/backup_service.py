...
def _openssl_encrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("Current environment is missing openssl, cannot perform encrypted backup") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Encryption backup failed: {detail or 'openssl execution failed'}") from exc
...
def _openssl_decrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("Current environment is missing openssl, cannot decrypt backup contents") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Decryption backup failed: {detail or 'openssl execution failed'}") from exc
    return result.stdout
...
class CloudflareR2Client:
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise BackupError(f"R2 configuration incomplete: missing {', '.join(missing)}")
    ...
    def test_connection(self) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"Failed to connect to R2: HTTP {response.status_code}")
    ...
    def upload_bytes(self, key: str, payload: bytes, *, content_type: str, metadata: dict[str, str] | None = None) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"Failed to upload backup: HTTP {response.status_code}")
    ...
    def delete_object(self, key: str) -> None:
        ...
        if response.status_code >= 400 and response.status_code != 404:
            raise BackupError(f"Failed to delete backup: HTTP {response.status_code}")

    def download_bytes(self, key: str) -> bytes:
        response = self._request("GET", key, timeout=60.0)
        if response.status_code >= 400:
            raise BackupError(f"Failed to read backup: HTTP {response.status_code}")
    ...
    def list_objects(self) -> list[dict[str, object]]:
        ...
        while True:
            ...
            if response.status_code >= 400:
                raise BackupError(f"Failed to get backup list: HTTP {response.status_code}")
            text = response.text
...
class BackupService:
    ...
    def download_backup(self, key: str) -> dict[str, object]:
        ...
        if candidate.endswith(".enc"):
            ...
            if not passphrase:
                raise BackupError("No encryption passphrase configured, cannot download and decrypt encrypted backup")
    ...
    def run_backup(self, *, trigger: str = "manual") -> dict[str, object]:
        with self._lock:
            ...
            if self._running:
                raise BackupError("A backup task is already running")
            started_at = _iso_now()
            self._running = True
            save_backup_state({
    ...
    def _run_backup_once(self, *, trigger: str) -> dict[str, object]:
        ...
        if encrypted:
            ...
            if not passphrase:
                raise BackupError("Backup encryption is enabled, but no passphrase is set")
    ...
    def _decode_backup_payload(self, key: str, payload: bytes) -> dict[str, object]:
        ...
        if key.endswith(".enc"):
            passphrase = _clean(config.get_backup_settings().get("passphrase"))
            if not passphrase:
                raise BackupError("No encryption passphrase configured, cannot view encrypted backup")
    ...
    def _decode_archive_detail(self, payload: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise BackupError("Failed to parse backup archive, backup might be corrupted") from exc
