...
def _public_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if task.get("status") in (TASK_STATUS_RUNNING, TASK_STATUS_QUEUED):
        if task.get("status") == TASK_STATUS_RUNNING:
            # RUNNING state is only timed after started_ts is set (image_stream_resolve_start)
            base_ts = task.get("started_ts")
        else:
            # QUEUED state is timed from created_ts (queued/waiting)
...
class ImageTaskService:
    ...
    def _run_task(
        ...
        # Create progress callback to update task status after each step completes
        def progress_callback(step: str) -> None:
            if step == "image_stream_resolve_start":
                self._update_task(key, started_ts=time.time())
            self._update_task(key, progress=step)
        # Add progress callback to payload (handler will extract and pass to ConversationRequest)
        ...
        try:
            ...
            if not isinstance(data, list) or not data:
                ...
                if upstream:
                    ...
                    message = "No available accounts in pool or all accounts are rate-limited. Please check account pool status (quotas, bans, limits)."
    ...
    def _log_call(
        ...
        *,
        ...
        summary_prefix = "Image-to-Image" if mode == "edit" else "Text-to-Image"
        detail = {
    ...
    def resume_poll(
        ...
        """Resumes polling for timed-out tasks, waiting for an additional extra_timeout_secs."""
        owner = _owner_id(identity)
        ...
        with self._lock:
            ...
            if "timeout" not in error_msg.lower() and "超时" not in error_msg:
        ...
        # Start a new thread to continue polling
        thread = threading.Thread(
    ...
    def _run_resume_poll(
        ...
        """Background thread: continues polling for image results with an existing conversation_id."""
        started = time.time()
        backend = None
        try:
            ...
            if not file_ids and not sediment_ids:
                raise RuntimeError(
                    f"Continued waiting for {extra_timeout_secs} seconds but still no image results were found."
                )
            ...
            if not image_urls:
                raise RuntimeError("Image URL parsing failed")

            image_items = [
                {"b64_json": __import__("base64").b64encode(image_data).decode("ascii")}
                for image_data in backend.download_image_bytes(image_urls)
            ]
            # Get the original prompt of the task (inferred from the mode of _public_task)
            with self._lock:
                task = self._tasks.get(key)
            ...
            data = format_image_result(
                image_items,
                "",  # prompt is no longer important as the result is already obtained
                "b64_json",
                "",
                int(time.time()),
            )["data"]
            ...
            self._log_call(
                ...
                model,
                started,
                "Call completed (resumed polling)",
                status="success",
                urls=_collect_image_urls(data),
            )
        except Exception as exc:
