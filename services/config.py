...
def _validate_image_storage_settings(settings: dict[str, object]) -> None:
    ...
    if not str(settings.get("webdav_url") or "").strip():
        raise ValueError("WebDAV URL must be specified when WebDAV image storage is enabled")
    if not str(settings.get("webdav_password") or "").strip():
        raise ValueError("WebDAV password must be specified when WebDAV image storage is enabled")
...
def _load_settings() -> LoadedSettings:
    ...
    if _is_invalid_auth_key(auth_key):
        raise ValueError(
            "❌ auth-key not set!\n"
            "Please set it in the CHATGPT2API_AUTH_KEY environment variable or fill in auth-key in config.json."
        )
...
class ConfigStore:
    def __init__(self, path: Path):
        ...
        if _is_invalid_auth_key(self.auth_key):
            raise ValueError(
                ...
                "1. Add in Render Environment variables:\n"
    ...
    def image_settle_enabled(self) -> bool:
        """Image double confirmation mechanism: wait for a period of time after finding file_ids before confirming again."""
    ...
    def image_check_before_hit_enabled(self) -> bool:
        """Check before hit: confirm that file_ids exist via polling before returning, rather than relying solely on SSE events."""
    ...
    def image_remove_conversation_after_result(self) -> bool:
        """Asynchronously hide ChatGPT local conversation history after successful image generation."""
    ...
    def image_settle_secs(self) -> float:
        """Wait time for double confirmation (seconds)."""
        try:
    ...
    def get_storage_backend(self) -> StorageBackend:
        """Gets storage backend instance (singleton)"""
