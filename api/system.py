...
<title>Giám sát sức khỏe Pool tài khoản - chatgpt2api</title>
<style>
...
<body>
<div class="header">
<h1><span class="status-dot {'status-ok' if healthy else 'status-degraded'}"></span>Giám sát sức khỏe Pool tài khoản</h1>
<div style="font-size:13px;color:#94a3b8">v{app_version} · Tự động làm mới sau 30 giây</div>
</div>
<div class="container">
<div class="cards">
<div class="card"><div class="label">Trạng thái Pool</div><div class="value {'green' if healthy else 'yellow'}">{'Bình thường' if healthy else 'Bất thường'}</div></div>
<div class="card"><div class="label">Tài khoản hiện tại</div><div class="value blue">{stats['total']}</div></div>
<div class="card"><div class="label">Tích lũy nhập kho</div><div class="value">{stats['cumulative_total']}</div></div>
<div class="card"><div class="label">Tài khoản khả dụng</div><div class="value green">{stats['active']}</div></div>
<div class="card"><div class="label">Không giới hạn</div><div class="value">{stats['unlimited_quota_count']}</div></div>
<div class="card"><div class="label">Hạn ngạch còn lại</div><div class="value">{stats['total_quota']}</div></div>
<div class="card"><div class="label">Bị giới hạn</div><div class="value yellow">{stats['limited']}</div></div>
<div class="card"><div class="label">Bất thường</div><div class="value red">{stats['abnormal']}</div></div>
<div class="card"><div class="label">Bị vô hiệu hóa</div><div class="value">{stats['disabled']}</div></div>
<div class="card"><div class="label">Thành công/Thất bại</div><div class="value">{stats['total_success']}<span style="font-size:18px;color:#94a3b8">/</span><span class="red">{stats['total_fail']}</span></div></div>
</div>
<h2 style="margin-bottom:12px;font-size:16px">Phân phối loại tài khoản</h2>
<table>
