...
function normalizeThirdPartyApps(value: unknown): ThirdPartyAppsSettings {
  ...
  return {
    infinite_canvas: {
      enabled: Boolean((canvas as any).enabled),
      url: String((canvas as any).url || DEFAULT_THIRD_PARTY_APPS.infinite_canvas.url),
    },
  };
}
...
type SettingsStore = {
  ...
  setRegisterProxyRotatingEnabled: (value: boolean) => void;
  setRegisterProxyRotatingKeysText: (value: string) => void;
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  testImageStorage: async () => {
    ...
    try {
      ...
      if (data.result.ok) {
        toast.success(`WebDAV connection available: HTTP ${data.result.status}`);
      } else {
  ...
  syncImagesToWebDAV: async () => {
    ...
    try {
      ...
      toast.success(`Synchronization completed: uploaded ${data.result.uploaded}, skipped ${data.result.skipped}, failed ${data.result.failed}`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to synchronize images");
    } finally {
  ...
  setRegisterProxyRotatingEnabled: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, proxy_rotating_enabled: value } } : {});
  },
  setRegisterProxyRotatingKeysText: (value) => {
    const keys = value.split("\n").map((item) => item.trim()).filter(Boolean);
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, proxy_rotating_keys: keys } } : {});
  },
  ...
  saveRegister: async () => {
    ...
    try {
      ...
      const data = await updateRegisterConfig({
        ...
        proxy_rotating_enabled: Boolean(registerConfig.proxy_rotating_enabled),
        proxy_rotating_keys: registerConfig.proxy_rotating_keys || [],
  ...
  resetOutlookPool: async (scope) => {
    ...
    try {
      ...
      set({ registerConfig: data.register });
      toast.success(scope === "unused" ? "Unused mailboxes deleted" : scope === "failed" ? "Failed/occupied mailbox status deleted" : "All Outlook email pool statuses reset");
    } catch (error) {
  ...
  browseFiles: async (pool) => {
    ...
    try {
      ...
      toast.success(`Successfully read ${files.length} remote accounts`);
    } catch (error) {
